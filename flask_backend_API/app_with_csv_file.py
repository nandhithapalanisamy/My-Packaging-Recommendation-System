...
def recommend_material_data(weight, fragility, baseline_only=None):
    ...
    # --------------------------------
    if baseline_only:
        # baseline_only will already be a dictionary row
        materials = [baseline_only]
    else:
        # Convert entire CSV dataframe to list of dictionaries
    ...
    for m in materials:
        ...
        # --------------------------------
        material_capacity = float(m.get("weight_capacity", 0.0))
        material_strength = float(m.get("strength", 0.0))
        ...
        recommendations.append({
            "material": decode_material_type(pd.Series(m)),
            "predicted_cost": round(cost_inr, 2),
...
def material_comparison():
    ...
    # -----------------------------
    # Find Baseline from CSV
    # -----------------------------
    baseline_row = None

    for _, row in materials_df.iterrows():
        if decode_material_type(row).lower() == baseline_name.lower():
    ...
    baseline_cost = baseline.get("predicted_cost", 0)
    baseline_co2 = baseline.get("predicted_co2", 0)
...
def material_performance():
    ...
    for _, row in materials_df.iterrows():
        material_name = decode_material_type(row)
        if material_name == selected_material:
            material_row = row.to_dict()
            break
...
def sustainability_report():
    ...
    for _, row in materials_df.iterrows():
        ...
        sustainability_score = (
            float(m.get("biodegradability_score", 0)) * 25 +
            float(m.get("recyclability", 0)) * 20 +
            (1 - float(m.get("co2_impact_index", 0))) * 25 +
            float(m.get("cost_efficiency_index", 0)) * 20 +
            float(m.get("material_suitability_score", 0)) * 10
        )

        report.append({
            "material": material_name,
            "sustainability_score": round(sustainability_score, 2),
            "co2_impact": round(float(m.get("co2_impact_index", 0)) * 100, 2),
            "cost_efficiency": round(float(m.get("cost_efficiency_index", 0)) * 100, 2)
        })
